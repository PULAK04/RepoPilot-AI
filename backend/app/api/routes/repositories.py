...
from app.workers.tasks import sync_repository_task

router = APIRouter(prefix="/repositories", tags=["repositories"])


@router.post("", response_model=RepositoryOut, status_code=201)
def create_repository(payload: RepositoryCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    try:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Could not access repository: {exc}")

    branch = payload.branch or info.get("default_branch") or "main"
    existing = db.scalar(select(Repository).where(
